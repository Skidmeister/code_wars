...
def check_1_9(l: list) -> bool:
    '''returns true if all numbers are from 1 to 9 and if differ'''
    return sorted(l) == [1,2,3,4,5,6,7,8,9]


def validate_sudoku(board: list) -> bool:
    '''validates 2D 9x9 Sudoku board'''
    ...
    while True:
        ...
        for x in range(9):
            if answer == False:
                break
            answer = check_1_9([row[x] for row in board[:]])

        #checking boxes
        a = np.array(board, dtype=np.int64)
        for y in range(3):
            for x in range(3):
                if answer == False:
                    break
                small_arr= (a[(3*y):(3*(y+1)),(3*x):(3*(x+1))]).tolist()
                small_arr_list = [item for sublist in small_arr for item in sublist]
                answer = check_1_9(small_arr_list)
